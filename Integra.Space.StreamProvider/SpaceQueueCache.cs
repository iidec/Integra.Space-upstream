...
namespace Integra.Space.StreamProvider
{
	...
	internal class SpaceQueueCacheItem
	{
	...
	/// </summary>
	public class SpaceQueueCache : IQueueCache
	{
		private readonly LinkedList<SpaceQueueCacheItem> cachedMessages;
		...
		public SpaceQueueCache(int cacheSize, Logger logger)
		{
			cachedMessages = new LinkedList<SpaceQueueCacheItem>();
		...
		private List<IBatchContainer> DrainBucket(CacheBucket bucket)
		{
			...
			{
				SpaceQueueCacheItem item = cachedMessages.Last.Value;
		...
		public virtual IQueueCacheCursor GetCacheCursor(IStreamIdentity streamIdentity, StreamSequenceToken token)
		{
			...
			var cursor = new SpaceQueueCacheCursor(this, streamIdentity, logger);
			InitializeCursor(cursor, token, true);
			return cursor;
		}

		internal void InitializeCursor(SpaceQueueCacheCursor cursor, StreamSequenceToken sequenceToken, bool enforceSequenceToken)
		{
			...
			LinkedListNode<SpaceQueueCacheItem> lastMessage = cachedMessages.Last;
			...
			LinkedListNode<SpaceQueueCacheItem> node = cachedMessages.First;
		...
		/// <returns></returns>
		internal bool TryGetNextMessage(SpaceQueueCacheCursor cursor, out IBatchContainer batch)
		{
		...
		private void UpdateCursor(SpaceQueueCacheCursor cursor, LinkedListNode<SpaceQueueCacheItem> item)
		{
		...
		internal void SetCursor(SpaceQueueCacheCursor cursor, LinkedListNode<SpaceQueueCacheItem> item)
		{
		...
		internal void ResetCursor(SpaceQueueCacheCursor cursor, StreamSequenceToken token)
		{
		...
		private void Add(IBatchContainer batch, StreamSequenceToken sequenceToken)
		{
			...
			var item = new SpaceQueueCacheItem
			{
				Batch = batch,
			...
			cachedMessages.AddFirst(new LinkedListNode<SpaceQueueCacheItem>(item));
