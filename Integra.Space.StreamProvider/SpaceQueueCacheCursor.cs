...
{
	class SpaceQueueCacheCursor : IQueueCacheCursor
	{
		private readonly IStreamIdentity streamIdentity;
		private readonly SpaceQueueCache cache;
		...
		internal LinkedListNode<SpaceQueueCacheItem> Element { get; private set; }
		...
		internal void Set(LinkedListNode<SpaceQueueCacheItem> item)
		{
			Element = item;
		...
		public SpaceQueueCacheCursor(SpaceQueueCache cache, IStreamIdentity streamIdentity, Logger logger)
		{
			if (cache == null)
			{
			...
			this.logger = logger;
			current = null;
			SpaceQueueCache.Log(logger, "SpaceQueueCacheCursor New Cursor for {0}, {1}", streamIdentity.Guid, streamIdentity.Namespace);
		}
		...
		{
			SpaceQueueCache.Log(logger, "SpaceQueueCacheCursor.GetCurrent: {0}", current);
		...
		{
			return
				$"<SpaceQueueCacheCursor: Element={Element?.Value.Batch.ToString() ?? "null"}, SequenceToken={SequenceToken?.ToString() ?? "null"}>";
		}
	}
}
